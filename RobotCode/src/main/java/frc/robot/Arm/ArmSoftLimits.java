...
public class ArmSoftLimits {
    ...
    public boolean isLimited(){
        ...
        // and then sets the wanted position to the limit if it is outside the restriction area.
        double[] restrictionXPoints = {1,1,3,3}; //This is where we declare the points for the restriction area. RESTRICTION LINES CAN NOT BE VERTICLE OR HORIZONTLE
        double[] restrictionYPoints = {1,2,3,4};
        int loopForTestingSoftLimits = 0;

            double wantedYPosArm = 1; // These are temporary values, we will need to change them when we get the simulation done. 
        ...
        while(restrictionXPoints.length >= loopForTestingSoftLimits + 1) { // this loop will repeat for each side
            ...
            double restrictionPointOneX = restrictionXPoints[loopForTestingSoftLimits];// this sets the restriction points to the relevant value 
...
double restrictionPointTwoY;
            ...
            if (loopForTestingSoftLimits + 1 >= restrictionXPoints.length) { // this if statement checks to see if the realivant value for the restrictions is the last one on the list
                ...
                restrictionPointTwoX = restrictionXPoints[loopForTestingSoftLimits]; // if not it chooses the following value 
            ...
            if (  restrictionPointTwoX - restrictionPointOneX == 0) { // my attempt to try to prevent restrictionangle var being NaN :/ I dont know if it works yet
   slopeForRestrictionLine = (  restrictionPointTwoY - restrictionPointOneY)/( 0.01);
} else {
          
     slopeForRestrictionLine = (  restrictionPointTwoY - restrictionPointOneY)/(  restrictionPointTwoX - restrictionPointOneX);
}
            Boolean parrallelTestForLines = 0.1 > Math.sqrt((slopeForWantedLine - slopeForRestrictionLine) * (slopeForWantedLine - slopeForRestrictionLine)); // this sets a boolean var. to true if the two lines are close enough to parrallel
            if (parrallelTestForLines == false) { // if the two are not parrallel then it executes the following code:
             ...
             // finding the y pos where the two lines intercept 
                ...
                loopForTestingSoftLimits = loopForTestingSoftLimits + 1; // adds 1 to the number of times that the loop has run.
                }
            }
        }
